...
def archive_bom(concept, version):
    source = f"releases/test/{concept}.yaml"
    dest = f"releases/test/{concept}-{version}.yaml"
...
def main():
    parser = argparse.ArgumentParser(description="Archive Test BOM to a versioned release")
