...
def test_startup_logs(caplog):
    ...
    env_vars = {"ENVIRONMENT": "unit-test", "HOSTNAME": "test-host-123", "LOG_LEVEL": "DEBUG"}

    with mock.patch.dict(os.environ, env_vars):
        # Trigger startup via TestClient context manager
        # Capture logs MUST allow startup to happen inside
        with caplog.at_level(logging.DEBUG, logger="demo-app"), TestClient(app):
            pass
...
def test_startup_unknown_env(caplog):
    with caplog.at_level(logging.INFO, logger="demo-app"), TestClient(app):
        pass
