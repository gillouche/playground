...
def test_read_root_default():
    ...
    assert response.json() == {"message": "Hello, World! Welcome to the Playground."}

def test_read_root_with_name():
    response = client.get("/?name=Alice")
    assert response.status_code == 200
    assert response.json() == {"message": "Hello, Alice! Welcome to the Playground."}
...
def test_read_root_with_sanitization():
    ...
    assert response.json() == {"message": f"Hello, {expected_safe}! Welcome to the Playground."}

def test_read_root_whitespace():
    # Test whitespace stripping
    response = client.get("/?name=  Bob  ")
    assert response.status_code == 200
    assert response.json() == {"message": "Hello, Bob! Welcome to the Playground."}
